...
getSpecificSticker = async(req, res) => {
    ...
    try {
        ...
        if (!req.query.evaluation_by) {
            ...
            stickers = await Sticker.findAll({
                where: {
                    status: req.query.evaluation_by === "FIC" ?
                        "unapproved" : "approved by FIC",
                },
                attributes: [
                    ["sid", "id"],
                    ["userId", "user_id"],
    ...
    if (
        ...
        try {
            const resp = await Sticker.update({
                status: evaluatedBy === "FIC" ?
                    status === "approved" ? "approved by FIC" : "denied by FIC" : status === "approved" ? "approved by CSO" : "denied by CSO",
                reason,
                validity
            }, {
